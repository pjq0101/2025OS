...
alloc_proc(void)
{
    ...
    if (proc != NULL)
    {
        ...
        //LAB8 2311874 : (update LAB6 steps)
        /*
         * 需要在 proc_struct 中初始化的字段（LAB8 新增）：
         *       struct files_struct * filesp;  // 文件结构体指针，用于管理进程打开的文件
         */
...
void proc_run(struct proc_struct *proc)
{
    if (proc != current)
    {
        ...
        //LAB8 2311874 : (update LAB4 steps)
        /*
         * 在调用 switch_to() 之前需要执行的操作：
         *       在切换页表后需要刷新 TLB（Translation Lookaside Buffer）
         *       可用的宏或函数：
         *       flush_tlb():  刷新 TLB 缓存，确保新页表生效
         */
        bool intr_flag;
...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    ret = -E_NO_MEM;
    // LAB8:EXERCISE2 2312207  HINT:how to copy the fs in parent's proc_struct?
    // LAB4:填写你在lab4中实现的代码
    /*
     * Some Useful MACROs, Functions and DEFINEs, you can use them in below implementation.
     * MACROs or Functions:
...
bad_fork_cleanup_fs: // LAB8: 清理文件系统相关资源
    put_files(proc);
...
{
    /* LAB8:EXERCISE2 2311874  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
     ...
     */
    // 可以参考 LAB5 中已完成的代码来实现
    /* 实现步骤：
     * (1) 为当前进程创建新的内存管理结构 mm
     * (2) 创建新的页目录表 PDT，mm->pgdir 指向 PDT 的内核虚拟地址
     * (3) 将二进制文件中的 TEXT/DATA/BSS 部分复制到进程内存空间
     *    (3.1) 从文件中读取原始数据并解析 ELF 头
     *    (3.2) 根据 ELF 头信息读取并解析程序头
     *    (3.3) 调用 mm_map 建立与 TEXT/DATA 相关的虚拟内存区域
     *    (3.4) 调用 pgdir_alloc_page 为 TEXT/DATA 分配页，从文件读取内容
     *          并复制到新分配的页中
     *    (3.5) 调用 pgdir_alloc_page 为 BSS 段分配页，将这些页清零
     * (4) 调用 mm_map 设置用户栈，并将参数放入用户栈
     * (5) 设置当前进程的 mm、页表基址，重置页目录（使用 lsatp 宏）
