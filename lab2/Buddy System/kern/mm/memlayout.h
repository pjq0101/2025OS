...
struct Page {
    ...
    // Add extra members if needed by Buddy System
};

/* Flags describing the status of a page frame */
#define PG_reserved                 0       // if this bit=1: the Page is reserved for kernel, cannot be used in alloc/free_pages; otherwise, this bit=0 
#define PG_property                 1       // if this bit=1: the Page is the head page of a free memory block(contains some continuous_addrress pages), and can be used in alloc_pages; if this bit=0: if the Page is the the head page of a free memory block, then this Page and the memory block is alloced. Or this Page isn't the head page.
#define PG_buddy                    2       // if this bit=1: indicates this page is part of a Buddy System block
...
#define SetPageBuddy(page)          ((page)->flags |= (1UL << PG_buddy))
#define ClearPageBuddy(page)        ((page)->flags &= ~(1UL << PG_buddy))
#define PageBuddy(page)             (((page)->flags >> PG_buddy) & 1)
...
/* Buddy System相关结构 - 添加用于记录每个空闲块的管理结构 */
#define MAX_ORDER 14  // 假设最大块阶数为 14

// Buddy System 空闲区域管理结构
typedef struct {
    free_area_t buddy_free_area[MAX_ORDER]; // 每个阶级的空闲区域链表
} buddy_system_t;
