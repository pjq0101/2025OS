...
alloc_proc(void)
{
    ...
    if (proc != NULL)
    {
        ...
        // LAB5 2312207 : (update LAB4 steps)
        /*
...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    //    3. call copy_mm to dup OR s hare mm according clone_flag
    ...
    local_intr_save(intr_flag);
    {
        ...
        // LAB5修改：使用set_links而不是list_add
        // 原来的代码：list_add(&proc_list, &(proc->list_link)); nr_process++;
...
load_icode(unsigned char *binary, size_t size)
{
    ...
    for (; ph < ph_end; ph++)
    {
        ...
        if (start < la)
        {
            ...
            // page 应该已经在上面的 while 循环中初始化了
            // 但为了消除编译器警告，确保 page 不为 NULL
            if (page == NULL)
            {
                ret = -E_NO_MEM;
                goto bad_cleanup_mmap;
